 ...
 *
 * This file contains the customizer importer functionality.
 *
 * @package Conjure WP
 */
 ...
 * @package Conjure WP
 */
class Conjure_Customizer_Importer {
	/**
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		// Prevent object instantiation when importing serialized customizer data.
		$data = @unserialize( $raw, array( 'allowed_classes' => false ) );

		// Data checks.
		if ( ( false === $data && 'b:0;' !== trim( $raw ) ) || ! is_array( $data ) || ! isset( $data['template'], $data['mods'] ) || ! is_array( $data['mods'] ) ) {
			return new \WP_Error(
		...
		// Import images.
		if ( apply_filters( 'conjure_customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new Conjure_Customizer_Option(
					$wp_customize,
					$option_key,
					array(
						'default'    => '',
		...
		$use_wp_customize_save_hooks = apply_filters( 'conjure_enable_wp_customize_save_hooks', false );
	 ...
	 */
	private static function customizer_is_image_url( $str = '' ) {
		if ( is_string( $str ) ) {
