 ...
 *
 * This file contains the widget importer functionality.
 *
 * @package Conjure WP
 */
...
class Conjure_Widget_Importer {
	...
	public static function import( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			Conjure_Logger::get_instance()->error( $results->get_error_message() );
		...
		Conjure_Logger::get_instance()->debug( $message );
	...
	private static function import_data( $data ) {
		...
		// Hook before import.
		do_action( 'conjure_widget_importer_before_widgets_import', $data );
		$data = apply_filters( 'conjure_before_widgets_import_data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				$widget = apply_filters( 'conjure_widget_settings', $widget ); // Object.
				...
				$widget = apply_filters( 'conjure_widget_settings_array', $widget );
				...
				if ( ! $fail ) {
					...
					do_action( 'conjure_widget_importer_after_single_widget_import', $after_widget_import );
		...
		// Hook after import.
		do_action( 'conjure_widget_importer_after_widgets_import', $data );

		// Return results.
		return apply_filters( 'conjure_widget_import_results', $results );
	}
	...
	private static function available_widgets() {
		...
		return apply_filters( 'conjure_available_widgets', $available_widgets );
	}
	...
	private static function unset_default_widgets() {
		$widget_areas = apply_filters( 'conjure_unset_default_widgets_args', false );
