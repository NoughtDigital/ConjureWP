...
class Conjure_Step_Manager {
	...
	public function handle_step_reset() {
		...
		$step = isset( $_GET['conjure_reset_step'] ) ? sanitize_key( wp_unslash( $_GET['conjure_reset_step'] ) ) : '';

		// Verify nonce.
		$nonce = isset( $_GET['_conjure_nonce'] ) ? sanitize_text_field( wp_unslash( $_GET['_conjure_nonce'] ) ) : '';
		if ( ! $nonce || ! wp_verify_nonce( $nonce, 'conjure_reset_step_' . $step ) ) {
